...
public static class AvPlayerExports
{
    ...
    private static readonly bool AutoResumeOnPausedVideoPoll = string.Equals(
        Environment.GetEnvironmentVariable("SHARPEMU_AVPLAYER_AUTO_RESUME_ON_POLL"),
        "1",
        StringComparison.Ordinal);
    private static readonly bool DisableLooping = string.Equals(
        Environment.GetEnvironmentVariable("SHARPEMU_AVPLAYER_DISABLE_LOOPING"),
        "1",
        StringComparison.Ordinal);
    private static readonly bool SkipPlayback = string.Equals(
        ...
        "1",
        StringComparison.Ordinal);
    private static readonly double MaximumDecodeFramesPerSecond =
        double.TryParse(
            Environment.GetEnvironmentVariable("SHARPEMU_AVPLAYER_MAX_DECODE_FPS"),
            NumberStyles.Float,
            CultureInfo.InvariantCulture,
            out var configuredDecodeFps) && configuredDecodeFps >= 1
            ? configuredDecodeFps
            : double.PositiveInfinity;
    private static int _traceCount;
    private static int _audioOwnerTraceCount;
    ...
    private sealed class PlayerState : IDisposable
    {
        ...
        public double DecodeFramesPerSecond { get; set; } = 30.0;
        ...
        public int PausedVideoPolls { get; set; }
        public Queue<ulong> PendingEvents { get; } = new();
        public bool DeliveringEvents { get; set; }
        public ulong WrapperThis { get; set; }
        public byte LastNearEosFlag { get; set; }
    ...
    public static int AvPlayerStart(CpuContext ctx)
    {
        ...
        lock (StateGate)
        {
            ...
            TraceState(player, skipped ? "start_skipped" : "start");
        }
    ...
    public static int AvPlayerPause(CpuContext ctx)
    {
        ...
        lock (StateGate)
        {
            ...
            _ = ctx.TryReadUInt64(ctx[CpuRegister.Rsp], out var pauseReturnAddress);
            // The wrapper's Pause method frame is push rbp/rbx/rax, so its
            // own return address (the actual pause DECIDER) sits 4 slots up.
            _ = ctx.TryReadUInt64(ctx[CpuRegister.Rsp] + 0x20, out var pauseCallerAddress);
            var wrapperState = "?";
            if (ctx.TryReadUInt64(player.EventObject + 0x18, out var wrapperThis) &&
                wrapperThis != 0 &&
                ctx.TryReadUInt64(wrapperThis + 0x190, out var wrapperDuration))
            {
                _ = ctx.TryReadUInt64(wrapperThis + 0xC8, out var wrapperLastTs);
                byte ReadFlag(ulong address)
                {
                    var flag = new byte[1];
                    return ctx.Memory.TryRead(address, flag) ? flag[0] : (byte)0xFF;
                }
                wrapperState =
                    $"this=0x{wrapperThis:X} dur={wrapperDuration} last_ts={wrapperLastTs} " +
                    $"near_eos={ReadFlag(wrapperThis + 0x1FA)} loop={ReadFlag(wrapperThis + 0x20F)} " +
                    $"prepared={ReadFlag(wrapperThis + 0x209)} pending_play={ReadFlag(wrapperThis + 0x20C)} " +
                    $"clock_mode={ReadFlag(wrapperThis + 0x130)}";
            }
            TraceState(player, $"pause ret=0x{pauseReturnAddress:X16} caller=0x{pauseCallerAddress:X16} {wrapperState}");
        }
    ...
    public static int AvPlayerSetLooping(CpuContext ctx)
    {
        lock (StateGate)
        {
            ...
            var requestedLooping = ctx[CpuRegister.Rsi] != 0;
            player.Looping = requestedLooping && !DisableLooping;
            TraceState(
                player,
                $"looping requested={requestedLooping} effective={player.Looping}");
            return SetReturn(ctx, 0);
        }
    }
    ...
    public static int AvPlayerEnableStream(CpuContext ctx)
    {
        Console.Error.WriteLine(
            $"[AVPLAYER][CALL] enable_stream handle=0x{ctx[CpuRegister.Rdi]:X16} " +
            $"index={ctx[CpuRegister.Rsi]}");
    ...
    public static int AvPlayerGetStreamInfoEx(CpuContext ctx)
    {
        ...
        lock (StateGate)
        {
            ...
            BinaryPrimitives.WriteUInt32LittleEndian(info[8..], GetStreamType(ctx, streamIndex));
            if (streamIndex == 0)
            {
            ...
            Console.Error.WriteLine(
                $"[AVPLAYER][CALL] stream_info_ex handle=0x{ctx[CpuRegister.Rdi]:X16} " +
                $"index={streamIndex} type={GetStreamType(ctx, streamIndex)} size={info.Length}");
    ...
    public static int AvPlayerJumpToTime(CpuContext ctx)
    {
        lock (StateGate)
        {
            ...
            player.Started = true;
            player.PlaybackClock.Start();
            TraceState(player, $"jump time={ctx[CpuRegister.Rsi]}");
            return SetReturn(ctx, 0);
        }
    }
    ...
    {
        var ownerTrace = Interlocked.Increment(ref _audioOwnerTraceCount);
        if (ownerTrace <= 16)
        {
            var sampler = ctx[CpuRegister.Rbx];
            var frame = ctx[CpuRegister.Rbp];
            _ = ctx.TryReadUInt64(frame - 0x28, out var owner);
            _ = ctx.TryReadUInt64(owner + 0x30, out var ownerSampler);
            _ = ctx.TryReadUInt64(owner + 0x38, out var videoSink);
            _ = TryReadUInt32(ctx, videoSink + 8, out var strongReferences);
            _ = TryReadUInt32(ctx, videoSink + 0xC, out var weakReferences);
            _ = ctx.TryReadUInt64(sampler, out var samplerVtable);
            _ = ctx.TryReadUInt64(sampler + 0x30, out var playerHandle);
            _ = ctx.TryReadUInt64(sampler + 0x40, out var videoTimestamp);
            Console.Error.WriteLine(
                $"[AVPLAYER][OWNER] poll={ownerTrace} thread=0x{GuestThreadExecution.CurrentGuestThreadHandle:X16} " +
                $"frame=0x{frame:X16} owner=0x{owner:X16} sampler=0x{sampler:X16} " +
                $"owner_sampler=0x{ownerSampler:X16} sink=0x{videoSink:X16} " +
                $"strong={strongReferences} weak={weakReferences} vtable=0x{samplerVtable:X16} " +
                $"handle=0x{playerHandle:X16} video_ts={videoTimestamp}");
        }

        DeliverPendingEvents(ctx, ctx[CpuRegister.Rdi]);
        if (ownerTrace == 2 &&
            ctx.TryReadUInt64(ctx[CpuRegister.Rbp] - 0x28, out var forcedOwner) &&
            forcedOwner != 0)
        {
            var scheduler = GuestThreadExecution.Scheduler;
            string? error = null;
            var called = scheduler is not null &&
                scheduler.TryCallGuestFunction(
                    ...
                    0,
                    0,
                    "avplayer_tick_fetch_probe",
                    out error);
            Console.Error.WriteLine(
                $"[AVPLAYER][TICK_FETCH_PROBE] owner=0x{forcedOwner:X16} " +
                $"called={called} error='{error ?? "-"}'");
        }
    ...
    public static int AvPlayerStreamCount(CpuContext ctx)
    {
        lock (StateGate)
        {
            var count = Players.TryGetValue(ctx[CpuRegister.Rdi], out var player)
                ? player.HasAudio ? 2 : 1
                : InvalidParameters;
            Console.Error.WriteLine(
                $"[AVPLAYER][CALL] stream_count handle=0x{ctx[CpuRegister.Rdi]:X16} count={count}");
    ...
    public static int AvPlayerGetStreamInfo(CpuContext ctx)
    {
        ...
        lock (StateGate)
        {
            ...
            // Gen5 removed the trailing Gen4 start-time field from the legacy descriptor.
            var infoSize = (ctx.TargetGeneration & Generation.Gen5) != 0
                ? Gen5StreamInfoSize
                : Gen4StreamInfoSize;
            Span<byte> info = stackalloc byte[infoSize];
            info.Clear();
            BinaryPrimitives.WriteUInt32LittleEndian(info[0..], GetStreamType(ctx, streamIndex));
            if (streamIndex == 0)
            {
            ...
            BinaryPrimitives.WriteUInt64LittleEndian(info[24..], player.DurationMilliseconds);
            Console.Error.WriteLine(
                $"[AVPLAYER][CALL] stream_info handle=0x{ctx[CpuRegister.Rdi]:X16} " +
                $"index={streamIndex} type={GetStreamType(ctx, streamIndex)} size={info.Length}");
            if (!ctx.Memory.TryWrite(infoAddress, info))
            {
                return SetReturn(ctx, InvalidParameters);
    ...
    private static uint GetStreamType(CpuContext ctx, uint streamIndex)
    {
        // Gen5 reserves zero for unknown and shifts the Gen4 video/audio values by one.
        return (ctx.TargetGeneration & Generation.Gen5) != 0
            ? streamIndex + 1
            : streamIndex;
    }
    ...
    private static int AddSource(CpuContext ctx, string guestPath)
    {
        ...
        lock (StateGate)
        {
            ...
            player.DecodeFramesPerSecond = Math.Min(fps, MaximumDecodeFramesPerSecond);
            ...
            Trace(
                ...
                $"fps={fps:F3} decode_fps={player.DecodeFramesPerSecond:F3} " +
    ...
    private static int GetVideoData(CpuContext ctx, bool extended)
    {
        ...
        lock (StateGate)
        {
            ...
            if (player.Paused)
            {
                player.PausedVideoPolls++;
                if (player.PausedVideoPolls <= 8 ||
                    (player.PausedVideoPolls & (player.PausedVideoPolls - 1)) == 0)
                {
                    Trace(
                        $"video_poll_paused handle=0x{player.Handle:X16} " +
                        $"count={player.PausedVideoPolls} clock_ms={player.PlaybackClock.ElapsedMilliseconds}");
                }
                if (!AutoResumeOnPausedVideoPoll)
                {
                    // Real avplayer keeps returning the CURRENT (held) frame
                    // while paused: Unity's per-tick pump needs a successful
                    // GetVideoDataEx to create its video textures and show
                    // the poster frame, and its paused frame-stepping
                    // (resume -> pull one frame -> pause) builds on that.
                    // Returning false here starved the whole video pipeline
                    // (black menu TV / black intro movie).
                    if (player.LastGuestBuffer != 0 &&
                        WritePosterFrameInfo(ctx, player, infoAddress, extended))
                    {
                        return SetReturn(ctx, 1);
                    }
                ...
                // Diagnostic compatibility path: some Unity players keep
                // polling immediately after a pause and never issue Resume.
                // Continue from the frozen media clock so we can distinguish
                // an AvPlayer state-machine stall from a video-composition
                // failure without changing default pause semantics.
                player.Paused = false;
                player.PlaybackClock.Start();
                TraceState(player, "auto_resume_on_video_poll");
            }
            ...
            var fps = Math.Max(1.0, player.DecodeFramesPerSecond);
            var expectedFrame = (long)Math.Floor(player.PlaybackClock.Elapsed.TotalSeconds * fps);
            // Real avplayer only returns a frame once the playback clock has
            // reached that frame's timestamp; polls in between yield no data.
            // Delivering ahead-of-clock frames (one per poll) made Unity's
            // AV-sync pause the player until its clock caught up with the
            // frame timestamp — and with the clock frozen by that pause,
            // playback deadlocked (video stayed black while the game went on).
    ...
    private static bool EnsureDecoder(PlayerState player)
    {
        ...
        if (player.DecodeFramesPerSecond + 0.001 < player.FramesPerSecond)
        {
            startInfo.ArgumentList.Add("-vf");
            startInfo.ArgumentList.Add(
                $"fps={player.DecodeFramesPerSecond.ToString("0.###", CultureInfo.InvariantCulture)}");
        }
        ...
        try
        {
            ...
            player.PlaybackClock.Start();
            Trace(
                $"decoder_started pid={player.Decoder.Id} " +
                $"decode_fps={player.DecodeFramesPerSecond:F3} source='{player.SourcePath}'");
            return true;
        }
    ...
    private static bool EnsureAudioDecoder(PlayerState player)
    {
        ...
        var ffmpeg = FindFfmpeg();
        if (!player.HasAudio || ffmpeg is null || player.SourcePath is null)
        {
            return false;
        }
        ...
        bool extended)
    {
        ...
        // Gen5's linear video-plane descriptors round their row pitch to 256
        // pixels (1440 -> 1536, 720 -> 768). Unity derives the chroma-plane
        // pointer from the pitch we report below, so the guest buffer must use
        // the same padded luma/chroma rows. A tightly packed 1440-byte frame
        // made every row after the first sample from the wrong offset and left
        // the movie white/black while subtitles continued normally.
    ...
    /// <summary>
    /// Re-delivers the most recent decoded frame while the player is paused
    /// (real avplayer keeps returning the held frame). No decoder access, no
    /// buffer-ring advance — only the frame-info struct is rewritten.
    /// </summary>
    private static bool WritePosterFrameInfo(
        CpuContext ctx,
        PlayerState player,
        ulong infoAddress,
        bool extended)
    ...
    private static bool AllocateGuestVideoBuffers(CpuContext ctx, PlayerState player, int bufferSize)
    {
        ...
        if (!player.TextureAllocatorFailed && player.AllocateTextureCallback != 0 && scheduler is not null)
        {
            ...
            {
                var callOk = scheduler.TryCallGuestFunction(
                    ctx,
                    ...
                    out var error);
                Console.Error.WriteLine(
                    $"[AVPLAYER][INFO] texture_alloc_call index={index} ok={callOk} " +
                    $"buffer=0x{buffer:X16} error={error ?? "<none>"}");
                if (!callOk || buffer == 0)
                {
                    Console.Error.WriteLine(
    ...
    // Real avplayer posts state events from its own worker thread AFTER the
    // triggering API has returned. Synchronous delivery ran the guest's READY
    // handler inside sceAvPlayerAddSourceEx: the handler stored the stream
    // duration, then the wrapper's post-open reset zeroed that field again -
    // Unity immediately computed "near end of stream" and paused the video
    // forever (black menu TV / black intro ad). Queue events and deliver them
    // on the guest's next call into the library instead.
    ...
    private static void DeliverPendingEvents(CpuContext ctx, ulong handle)
    {
        ...
        WatchWrapperNearEos(ctx, player);
        lock (StateGate)
        {
            if (player.DeliveringEvents || player.PendingEvents.Count == 0)
            {
                return;
            }
    ...
    // Diagnostic: pinpoint the guest call between which Unity's near-EOS flag
    // (wrapper+0x1FA) flips, with the duration/last-ts context that gated it.
    private static void WatchWrapperNearEos(CpuContext ctx, PlayerState player)
    {
        if (player.WrapperThis == 0 &&
            player.EventObject != 0 &&
            ctx.TryReadUInt64(player.EventObject + 0x18, out var wrapperThis))
        {
            player.WrapperThis = wrapperThis;
        }
        ...
        var flag = new byte[1];
        if (!ctx.Memory.TryRead(player.WrapperThis + 0x1FA, flag) ||
            flag[0] == player.LastNearEosFlag)
        {
            return;
        }

        _ = ctx.TryReadUInt64(player.WrapperThis + 0x190, out var duration);
        _ = ctx.TryReadUInt64(player.WrapperThis + 0xC8, out var lastTs);
        Console.Error.WriteLine(
            $"[AVPLAYER][INFO] near_eos_flip handle=0x{player.Handle:X16} " +
            $"{player.LastNearEosFlag}->{flag[0]} dur={duration} last_ts={lastTs} " +
            $"clock_ms={player.PlaybackClock.ElapsedMilliseconds} " +
            $"video_index={player.NextFrameIndex}");
        player.LastNearEosFlag = flag[0];
    }
    ...
    private static void NotifyEvent(CpuContext ctx, PlayerState player, ulong eventId)
    {
        ...
        Trace(
            $"event handle=0x{player.Handle:X16} id={eventId} " +
            $"callback=0x{player.EventCallback:X16} result=0x{callbackResult:X16}");
    }
    ...
    private static void TraceState(PlayerState player, string transition)
    {
        Console.Error.WriteLine(
            $"[AVPLAYER][STATE] {transition} handle=0x{player.Handle:X16} " +
            $"started={player.Started} paused={player.Paused} eos={player.EndOfStream} " +
            $"clock_ms={player.PlaybackClock.ElapsedMilliseconds} " +
            $"video_index={player.NextFrameIndex} audio_index={player.NextAudioFrameIndex}");
    }
}
