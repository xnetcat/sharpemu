...
internal sealed class MetalGuestGpuBackend : IGuestGpuBackend
{
    ...
    private static MetalCompiledGuestShader? ResolveVertexShader(
        IGuestCompiledShader? vertexShader,
        IReadOnlyList<GuestVertexBuffer>? vertexBuffers,
        Func<IReadOnlyList<GuestVertexBuffer>, IGuestCompiledShader?>?
            deferredVertexCompiler)
    {
        var resolved = deferredVertexCompiler?.Invoke(
            vertexBuffers ?? Array.Empty<GuestVertexBuffer>()) ?? vertexShader;
        return resolved is null ? null : Msl(resolved);
    }
