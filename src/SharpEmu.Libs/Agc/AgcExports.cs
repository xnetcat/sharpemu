...
public static partial class AgcExports
{
        ...
        ulong submissionId)
    {
        ...
        void TriggerCompletionEvents()
        {
            ...
            if (_traceAgcLabels)
            {
                // queues=0 means the end-of-pipe interrupt reached no equeue at
                // all, i.e. the ident we publish does not match what the guest
                // registered through sceAgcDriverAddEqEvent. That is invisible
                // in the normal log yet is exactly what leaves an interrupt
                // thread parked forever.
                Console.Error.WriteLine(
                    $"[LOADER][WARN] agc.completion_event queue={queueName} " +
                    $"submission={submissionId} event=0x{completionEventId:X} queues={triggered}");
            }
        }
