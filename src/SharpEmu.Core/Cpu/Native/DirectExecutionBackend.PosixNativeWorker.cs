...
{
	// POSIX uses a native System V run loop. Reusing the Win64-shaped worker
	// loop through register-shuffling thunks puts CLR reverse P/Invokes, libc
	// waits and pthread startup under three different ABI stack contracts.
	// Keeping the entire foreign-thread loop in the host ABI gives pthread and
	// the CLR one stable native frame chain around each guest slice.
	private sealed unsafe class PosixNativeGuestExecutor : INativeGuestExecutor
	{
		private const uint LoopStubSize = 512;
		private const int InterruptedSystemCall = 4;
		private static readonly bool BlockRuntimeSuspendSignal =
			ShouldBlockRuntimeSuspendSignal();
		...
		private bool _runtimeSuspendSignalBlocked;
		private byte[]? _runtimeSignalMaskBeforeGuest;

		private PosixNativeGuestExecutor(DirectExecutionBackend backend)
		{
			_backend = backend;
		}

		public static PosixNativeGuestExecutor? TryCreate(DirectExecutionBackend backend)
		{
			...
			var executor = new PosixNativeGuestExecutor(backend);
		...
		private static bool ShouldBlockRuntimeSuspendSignal()
		{
			var configured =
				Environment.GetEnvironmentVariable("SHARPEMU_BLOCK_RUNTIME_SUSPEND_SIGNAL");
			...
			// sysctl.proc_translated is Apple's supported Rosetta detection
			// mechanism. Native x64 hosts do not need this guard, and leaving
			// their normal runtime suspension protocol untouched avoids making
			// a platform-specific workaround process-wide.
			var translated = 0;
		...
		private bool Initialize()
		{
			...
			// pthread enters with RSP = 8 (mod 16). Preserve the complete System V
			// nonvolatile set and align RSP to 16 before every call.
			Emit(0x53);                                      // push rbx
			Emit(0x55);                                      // push rbp
		...
		private static nint RunPrologue(nint executorHandle)
		{
			try
			{
				var executor = (PosixNativeGuestExecutor)GCHandle.FromIntPtr(executorHandle).Target!;
			...
			catch (Exception exception)
			{
				try
				{
					Console.Error.WriteLine(
						$"[LOADER][ERROR] Raw POSIX guest worker prologue failed: " +
		...
		private static void RunEpilogue(nint executorHandle, int nativeResult)
		{
			...
			catch (Exception exception)
			{
				...
				{
					Console.Error.WriteLine(
						$"[LOADER][ERROR] Raw POSIX guest worker epilogue failed: " +
						$"{exception.GetType().Name}: {exception.Message}");
				}
				catch
				{
		...
		private nint EnterRun()
		{
			...
			if (BlockRuntimeSuspendSignal &&
				!_runtimeSuspendSignalBlocked)
			{
				if (!TryBlockRuntimeSuspendSignal(
						out _runtimeSignalMaskBeforeGuest,
						out var signalMaskError))
				{
					throw new InvalidOperationException(
						$"failed to block the host runtime suspension signal: errno={signalMaskError}");
				}

				_runtimeSuspendSignalBlocked = true;
			}
			_runPrologueFailed = false;
			return _runEntryStub;
		}

		private static bool TryBlockRuntimeSuspendSignal(
			out byte[] previousMask,
			out int error)
		{
			previousMask = new byte[128];
			error = 0;
			byte* signalSet = stackalloc byte[128];
			new Span<byte>(signalSet, 128).Clear();
			fixed (byte* previous = previousMask)
			{
				if (sigemptyset(signalSet) != 0 ||
					sigaddset(signalSet, GetRuntimeSuspendSignal()) != 0)
				{
					error = Marshal.GetLastPInvokeError();
					return false;
				}

				// pthread APIs return their error number directly instead of
				// reliably publishing it through errno.
				error = pthread_sigmask(
					GetSignalBlockOperation(),
					signalSet,
					previous);
				return error == 0;
			}
		...
		private void RestoreRuntimeSignalMask()
		{
			if (!_runtimeSuspendSignalBlocked ||
				_runtimeSignalMaskBeforeGuest is not { } previousMask)
			{
				return;
			}
			...
			fixed (byte* previous = previousMask)
			{
				var error = pthread_sigmask(
					GetSignalSetMaskOperation(),
					previous,
					null);
				if (error != 0)
				{
					Console.Error.WriteLine(
						$"[LOADER][WARN] Raw POSIX guest worker failed to restore its host " +
						$"signal mask: errno={error}");
				}
			}
			_runtimeSuspendSignalBlocked = false;
			_runtimeSignalMaskBeforeGuest = null;
		}

		private static int GetRuntimeSuspendSignal() =>
			OperatingSystem.IsMacOS() ? 30 : 10;

		private static int GetSignalBlockOperation() =>
			OperatingSystem.IsMacOS() ? 1 : 0;

		private static int GetSignalSetMaskOperation() =>
			OperatingSystem.IsMacOS() ? 3 : 2;
		...
		private void ExitRun(int nativeResult)
		{
			...
			// The worker sleeps in libc between guest slices. Restore the mask
			// before returning to that idle loop so a runtime suspension cannot
			// be stranded on a worker which may not be scheduled again.
			RestoreRuntimeSignalMask();
		}
		...
		[DllImport("libc", SetLastError = true)]
		private static extern int pthread_sigmask(int how, void* set, void* oldSet);

		[DllImport("libc", SetLastError = true)]
		private static extern int sigemptyset(void* set);

		[DllImport("libc", SetLastError = true)]
		private static extern int sigaddset(void* set, int signal);
