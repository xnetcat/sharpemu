...
{
    [Fact]
    public void DeferredCubeDescriptorRetainsAllSixLayers()
    {
        // A linear 1x1 R8G8B8A8 cube at 0x1000. RDNA2 word4 stores the
        // last cube-map array slice, so five denotes six image layers.
        uint[] descriptor =
        [
            0x00000010,
            0x03800000,
        ...
        // Linear RDNA images align the row pitch to 64 texels.
        var memory = new FakeCpuMemory(0x1000, 6 * 64 * 4);

        Assert.True(
            AgcExports.TryResolveDeferredDrawTexture(
                descriptor,
                isStorage: false,
                mipLevel: 0,
                sampler: default,
        ...
        Assert.Equal(11u, texture.Type);
        Assert.Equal(6u, texture.Depth);
        Assert.Equal(6 * 64 * 4, texture.RgbaPixels.Length);
